...
public:
    ...
    void pointsBounds(float* center3, float* radius) const;   // 點雲質心 + RMS 半徑（環繞樞軸）
...
public:
    ...
    /// Render with custom intrinsics + output size (for portrait viewing).
    void renderFromPoseIntrinsics(const float camToWorld[16], float fx, float fy, float cx, float cy,
