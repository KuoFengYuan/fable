...
nonisolated struct CaptureConfig: Sendable {
    ...
    /// 允許 BA 改動位姿。**真正的決定權不在這裡，在保留集**（BundleAdjuster.kHoldoutGate）：
    /// 每次掃描各自判定，保留集有進步才套用，否則輸出維持 ARKit＋錨點修正的位姿。
    ///
    /// 這一欄只是硬總開關，給「完全不想讓 BA 碰位姿」的情況用。設 false 時 BA 仍會跑
    /// 並印出判定 —— 那個判定是把它重新打開的唯一證據來源。
    ///
    /// 為什麼決定權必須下放到每次掃描：兩份實機 log 給出相反結論，而它們不矛盾。
    /// 1cm 位姿誤差的重投影誤差 @0.5m 是 29px、@2m 只有 7px ——
    /// 近距離掃描位姿誤差遠大於觀測雜訊（實測保留集 -21%），
    /// 房間尺度則與深度取樣雜訊同量級（BA 只是把雜訊擬合得更好）。
    /// 這是幾何決定的，我挑任何固定值都會在另一半情況下挑錯。
