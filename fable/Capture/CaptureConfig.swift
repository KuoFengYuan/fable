...
nonisolated struct CaptureConfig: Sendable {
    ...
    ///
    /// 注意這只是**求解**的輪數；解出來要不要套用是 baApplyPoses 管的（預設不套用）。
    var baRounds = 10

    /// BA 解出來的位姿要不要真的套用。
    ///
    /// **預設 false —— BA 目前只量測、不改位姿。** 判定它擬合雜訊多於修正誤差：
    ///
    ///   · 實機修正量（中位數 1.0cm）與觀測雜訊同量級
    ///     （重投影中位數 7.68px ≈ 1.06cm、深度殘差 3.1cm）
    ///   · 深度項佔目標函數平方成本 76%，而那一項是純 LiDAR 量測雜訊
    ///   · 離線測試第 7 案重現了同一個指紋（深度佔 96%、10 輪跑滿不回退、
    ///     目標函數「改善」2%），而它的保留集是 **+11%** —— 位姿實際上變差了
    ///   · 使用者實機比對 3DGS 結果：加了 BA 之後精度沒有變好
    ///
    /// **為什麼不乾脆把 baRounds 設 0。** 那會連「這個開關該不該打開」都量不到，
    /// 變成一扇單向門。保持 rounds > 0 而 apply = false，每次掃描仍會印出保留集判定
    /// （見 BundleAdjuster.kHoldoutEvery），成本毫秒級 ——
    /// 而它是唯一能拿證據把這個開關重新打開的來源。
    ///
    /// 打開的條件：保留集重投影中位數穩定下降 >3%。
    /// 在那之前最該做的是把觀測雜訊壓下去（特徵密度只有每幀 259 個，
    /// 平均最近鄰 52px vs 搜尋半徑 28px）—— 觀測比要修的誤差還吵，
    /// 再多的迭代也只是把雜訊擬合得更好。
    var baApplyPoses = false
